...
outputdir='../RESULTS/6Sauvola/single/';
extgroup={'.tif','.bmp','.jpg','.png','.gif','.jpeg'};
timefile='../RESULTS/6Sauvola-single.time';
...
for i = 1:length(extgroup)
    ...
    for j=1:length(files)
        ...
        sauvola_entrance(img_color,imgname,ext,outputdir);
        end
        timgtime=toc(timg);%t end
        fprintf(timefid,'%10s\tSauvola adaptive thresholding: %9.5f\n',imgname,timgtime);
     end
end
